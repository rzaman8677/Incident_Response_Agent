...
def settings_from_env(*, autonomy_mode: AutonomyMode | None = None) -> Settings:
    ...
    mode = autonomy_mode or AutonomyMode(
        os.getenv("SENTINELOPS_AUTONOMY", AutonomyMode.ASSISTED.value).strip().lower()
    )
    return Settings(
        autonomy_mode=mode,
        autonomous_confidence_threshold=float(
            os.getenv("SENTINELOPS_CONFIDENCE_THRESHOLD", "0.82")
        ),
        ...
        verification_attempts=max(
            1, int(os.getenv("SENTINELOPS_VERIFICATION_ATTEMPTS", "6"))
        ),
        verification_interval_seconds=max(
            0.0,
            float(os.getenv("SENTINELOPS_VERIFICATION_INTERVAL_SECONDS", "1")),
        ),
    )
